...
class CNNBLSTM:
    ...
    def createNetwork(self, configfile):
        ...
        with self.hybridmodel.as_default():
            ...
            ##################################inception block res connection ####################################
            #bn_nl = tf.layers.batch_normalization(nl, training=True)

            f1_double = [1, 1, 4, 24]
            W1_double = tf.Variable(tf.truncated_normal(f1_double, stddev=0.1), name="W1_double")
            b1_double = tf.Variable(tf.constant(0.1, shape=[24]), name="b1_double")
            conv_2_res = tf.nn.conv2d(nl, W1_double, strides=[1, 1, 1, 1], padding='SAME')
            nl_bn0 = tf.layers.batch_normalization(tf.nn.relu(tf.add(conv_2_res, b1_double)),training=True)
            shape = self.get_layer_shape(nl_bn0)
            #print("Res1 Block with Inception= ", shape)

            nl_res=tf.add(nl_bn0, merge_conv1)
            shape = self.get_layer_shape(nl_res)
            print("Res Block with Inception= ", shape)
            ##########################################end########################################################
            ...
            conv_3 = tf.nn.conv2d(nl_res, W3, strides=[1, self.filterheight, 2, 1], padding='SAME', name="3rd_CNN_block")
    ...
    def trainNetwork(self, nb_epochs, batchsize, x, y, transcription_length, weightfiles,
        ...
        with tf.Session(graph=self.hybridmodel, config=tf.ConfigProto(gpu_options=gpu_memory)) as session:
            ...
            for e in range(nb_epochs):
                ...
                # shape = cnn_0_out.shape
                # ind = 1
                # r = np.random.randint(shape[0])
                # for ch in range(shape[-1]):
                #     # for j in range(2):
                #     img = np.squeeze(cnn_0_out[r][:, :, ch:ch + 1], -1)
                #     # print("Image shape ",img.shape)
                #     ax = figure.add_subplot(shape[-1], 1, ind)
...
def main(task, mode, dbfile, files, weightfile, batchsize):
    ...
    if (task == "Train"):
        ...
        # train network
        model.trainNetwork(3000, batchsize, x, y, transcription_length, weightfiles, mode)
    elif (task == "Predict"):
        # test network
        model = CNNBLSTM(1, 353, "Hybrid")
        model.createNetwork("Config")
        #x_test = np.asarray(x_test)
        weightfile_best = weightfile + "/Best/residin_best"
        #weightfile_best = weightfile + "/Best/orig_best" for v0 model
        #for residual model use /Best/residin_best
...
dir = "/media/cvpr/Data1/Shahil/DegradedOCR_V2.0"
...
# files = ["/media/cvpr/Data2/PSM/Degraded-OCR/Train-Crp.h5", "/media/cvpr/Data2/PSM/Degraded-OCR/Test-Crp.h5"]
...
main(task, mode, dbfile, files, weightfile_name, batchsize)
